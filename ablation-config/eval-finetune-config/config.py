...
import data.genesis_multi_division_ghsv2_airstore_dlrefactor_finetune_eval as datamodel
...
#heldoutcams = []
holdoutcams = ['cam401036', 'cam401044','cam401067', 'cam401384', 'cam401412', 'cam401042']  # GHS v2 # UNLIKE AIRSTORE, no 'cam' prefix
holdoutseg = ["EXP_free_face"] # GHS v2
...
class ProgressWriter():
    def batch(self, iternum, itemnum, imagename = None, **kwargs):
        ...
        #print(" length of row {}".format(len(row)))

        if kwargs["image"].size(0) == 1:
            imgout = row[0]* 2.
            #imgout2 = row[0]
        else:
            imgout = np.concatenate(rows, axis=0) * 2.
        ...
        if imagename is None:
            ...
            Image.fromarray(np.clip(imgout, 0, 255).astype(np.uint8)).save(os.path.join(outpath, imagename))
            #Image.fromarray(np.clip(imgout2, 0, 255).astype(np.uint8)).save(os.path.join(outpath, imagename+"no2.png"))
