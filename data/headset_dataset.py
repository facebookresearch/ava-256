...
class SingleCaptureDataset(torch.utils.data.Dataset):
    ...
    def get_img_by_segment_and_frame(self, segment: str, frame: int) -> np.ndarray:
        ...
        return (torch.from_numpy(imgs_arr) / 255.0 - 0.5) * 2
    ...
    def get_assets_by_id(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.load_latent_code:
            ...
            try:
                latent_code = pickle.loads(
                    ZipPath(self.latent_code_dir / "rosetta_correspondences.zip", entry_name).read_bytes()
                )["expression"]
        ...
        return {
            ...
            "index": {"ident": self.capture.sid, "segment": segment_name, "frame": frame},
        }
