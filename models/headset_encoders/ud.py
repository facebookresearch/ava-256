...
class FixedFrontalViewGenerator(torch.nn.Module):
    ...
    def __init__(self, down_sample_factor: int = 2) -> None:
        ...
        self.register_buffer(
            "frontal_cam_pose", torch.tensor([[115.3817, 26.1830, 984.4196]]), persistent=False
        )  # 1 x 3
        self.register_buffer(
            "frontal_cam_rot",
            torch.tensor(
        ...
        self.register_buffer("frontal_princpt", torch.tensor([[751.2227, 967.2575]]) / 2, persistent=False)  # 1 x 2

    def forward(
        self, batch_size: int, randomize: bool, rvec: Optional[torch.Tensor] = None
...
class UDWrapper(torch.nn.Module):
    ...
    def __init__(self, ud_exp_name: str, identities: List[str]) -> None:
        super().__init__()
        ...
        for template in self.conditioning_fn_templates:
            ...
            for fn in fns:
                try:
                    ident_cond.append(
                        torch.load(fn, map_location=f"cuda:{dist.get_rank() if dist.is_initialized() else 0}")
                    )
            ...
            if any(x is None for x in ident_cond):
                missing_idents = [ident for ident, cond in zip(identities, ident_cond) if cond is None]
                raise FileNotFoundError(f"No available conditioning data for {', '.join(missing_idents)}")
            break
        else:
        ...
        for idx in range(self.iden_b_cond_len):
            self.register_buffer(f"b_geo_{idx}", torch.cat([cond["b_geo"][idx] for cond in ident_cond], 0))
            self.register_buffer(f"b_tex_{idx}", torch.cat([cond["b_tex"][idx] for cond in ident_cond], 0))
    ...
    def get_id_cond(
        self, idents: List[str], device: torch.device
    ...
    def render(self, inputs: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        if (
            ...
            px, py = np.meshgrid(np.arange(render_w).astype(np.float32), np.arange(render_h).astype(np.float32))
            # 1 * h * w * 2
...
if __name__ == "__main__":
    ...
    test_identities = ["XUD838_20230811--0000", "YYA788_20230718--0000"]

    start = time.perf_counter()
    uca1 = UDWrapper(ud_exp_name="/uca/leochli/oss/ava256_universal_decoder", identities=test_identities)
