...
class UniversalEncoder(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_cond: Optional[torch.Tensor] = None) -> Dict[str, Union[torch.Tensor, List[torch.Tensor]]]:
        """
        ...
        if num_conds > 0:
            ...
            assert eye_cond.shape[2] == self.num_conditions, f"Conditioning shape mismatch! Got {eye_cond.shape}, expect {self.num_conditions}"
            ...
            eye_features = torch.cat(
                [eye_features, eye_cond_features_pre], dim=1
            )
            face_features = torch.cat(
                [face_features, face_cond_features_pre], dim=1
            )
            # [Batch*Views, C, feat_H, feat_W]
        ...
        eye_features = F.adaptive_avg_pool2d(
            self.eye_model_postcond(eye_features), output_size=1
        ).squeeze(-2,-1)
        eye_features = rearrange(eye_features, "(n v) c -> n (v c)", v=(self.num_views // 2))
        face_features = F.adaptive_avg_pool2d(
            self.face_model_postcond(face_features), output_size=1
        ).squeeze(-2,-1)
