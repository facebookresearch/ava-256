...
class PatchMixAugmentation:
    ...
    def corrupt(self, feature: torch.Tensor, pfeature: torch.Tensor) -> torch.Tensor:
        ...
        h = (
            torch.stack(
                [
                    torch.randint(low=0, high=width, size=(bsz, num_views, self.num_holes), device=device),
                    torch.randint(low=0, high=height, size=(bsz, num_views, self.num_holes), device=device),
                ],
                dim=-1,
            )
            .float()
        ...
        d = (
            ((g.view(1, 1, 1, height, width, 2) - h).abs() < (self.size // 2))
            .float()
            .prod(dim=-1)
            .sum(dim=3)
            .clip(max=1.0)
