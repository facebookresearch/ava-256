...
class CoordConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        x = torch.cat([x,
            ...
            torch.linspace(-1., 1., x.size(3), device=x.device)[None, None, None, :].repeat(x.size(0), 1, x.size(2), 1)], dim=1)
...
class Conv2dWS(nn.Conv2d):
    ...
    def standardize_weights(self, eps):
        ...
        scale = 1.414 * torch.rsqrt(torch.max(
...
class ConvTranspose3dWS(nn.ConvTranspose3d):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        super(ConvTranspose3dWS, self).__init__(in_channels, out_channels, kernel_size, stride=stride,
    ...
    def standardize_weights(self, eps):
        ...
        scale = 1.414 * 2. * torch.rsqrt(torch.max(
            var * fan_in, torch.tensor(eps).to(var.device))) * self.gain.view_as(var).to(var.device)
        #shift = mean * scale
        #return self.weight * scale - shift
        return (self.weight - mean) * scale
    ...
    def forward(self, x):
        weight = self.standardize_weights(1e-4)
        out = F.conv_transpose3d(x, weight, self.bias, self.stride, self.padding, self.output_padding, self.groups, self.dilation)
        #print("@", out.mean().item(), out.std().item())
        return out
...
class LightEnvmapDecorator(nn.Module):
    def __init__(self, mod, dataset, L, lightinitpath=None):
        ...
        if lightinitpath is not None:
            ...
            self.light = nn.ParameterList([nn.Parameter(0.1 * torch.randn(1, L, 2*L)) for i in range(len(dataset.get_light_ids()))])
...
class LightEnvmapRotateDecorator(nn.Module):
    def __init__(self, mod, period=200.):
    ...
    def forward(self, lightid, **kwargs):
        ...
        for i in range(campos.size(0)):
            ...
            ph0 = (iternum * campos.size(0) + i) * 2. * 3.1415926 / self.period
            ...
            for y in range(4):
                for x in range(8):
                    th = ((y+0.5) / 4.) * 3.1415926
                    ph = ((x+0.5) / 4.) * 3.1415926
            ...
            light0 = light0 + 3. - torch.log(torch.sum(torch.exp(light0)))
            light.append(light0)
...
def sh(m, n, ph, th):
    ...
    if m < 0:
        return (np.sqrt(2.) * (-1 ** m) * np.imag(scipy.special.sph_harm(-m, n, ph, th))).astype(np.float32)
    elif m == 0:
        return (np.real(scipy.special.sph_harm(0, n, ph, th))).astype(np.float32)
    else:
        return (np.sqrt(2.) * (-1 ** m) * np.real(scipy.special.sph_harm(m, n, ph, th))).astype(np.float32)
    ...
    def __init__(self, mod, dataset, ncoeffs, lightinitpath=None):
        ...
        if lightinitpath is not None:
            ...
            th, ph = np.meshgrid(
                    ...
                    (np.arange(2 * L, dtype=np.float32) + 0.5) * 3.1415926 / L, indexing='ij')
            ...
            coeffs = torch.stack([
                torch.stack([
                    ...
                    for j in range(bases.size(0))], dim=0)
                for i in range(lightinit.size(0))], dim=0)
            ...
            self.light = nn.ParameterList([nn.Parameter(0.1 * torch.randn(ncoeffs)) for i in range(len(dataset.get_light_ids()))])
...
class SHNearfieldLightDecorator(nn.Module):
    ...
    def forward(self, iternum, lossweights, lightid, **kwargs):
        ...
        vec = torch.stack([torch.sin(th) * torch.sin(ph), torch.cos(th), torch.sin(th) * torch.cos(ph)], dim=-1) # [2L^2, 3]
        xyz = torch.stack([xx, yy, zz], dim=-1) # [D, D, D, 3]
        for i in range(lightid.size(0)):
            ph0 = (iternum * lightid.size(0) + i) * 2. * 3.1415926 / 131.
            ...
            light0 = torch.exp(-16.0 * torch.acos(dp.clamp(min=-0.999, max=0.999)) ** 2) * torch.sin(th[:, None, None, None])
            light0 = light0 / torch.sum(light0, dim=0, keepdim=True)
            light0 = light0 * 4. / lightdirnorm[None, :, :, :] ** 2
            ...
            #    for i in range(self.bases.size(0))], dim=0)
            light0 = torch.sum(light0[None, :, :, :, :] * self.bases[:, :, None, None, None], dim=1) * (np.pi ** 2 / (2 * self.L ** 2))
