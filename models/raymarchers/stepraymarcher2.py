...
class Raymarcher(nn.Module):
    def __init__(self, volsampler, volradius):
    ...
    def forward(self, viewpos, viewrot, raydir, decout,
        ...
        with torch.no_grad():
            # TODO: volbounds
            t1 = (-self.volradius - viewpos[:, None, None, :]) / raydir
            t2 = ( self.volradius - viewpos[:, None, None, :]) / raydir
        ...
        while not done.all():
            ...
            validf = torch.prod(torch.gt(raypos, -1.0) * torch.lt(raypos, 1.0), dim=-1).float()
            #validf = valid.float()
            ...
            raystep = renderoptions["dt"] * jitter
            step = (renderoptions["dt"] / self.volradius) * jitter

            if "terminateatmesh" in renderoptions and renderoptions["terminateatmesh"]:
                done = done | ((t + raystep) >= tminmax[..., 1]) | (raydepth >= rastdepth)
            else:
                done = done | ((t + raystep) >= tminmax[..., 1])
            ...
            raypos = raypos + raydir * raystep[:, :, :, None]
            t = t + raystep
            #i += 1
        ...
        return (rayrgb, rayalpha), rayposend
