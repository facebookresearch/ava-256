...
class Raymarcher(nn.Module):
    ...
    def forward(self, raypos, raydir, tminmax, decout, encoding=None, renderoptions={}, **kwargs):
        ...
        while not done.all():
            valid = torch.prod((raypos > -1.0) * (raypos < 1.0), dim=-1).float()
            samplepos = F.grid_sample(decout["warp"][:, 0], raypos[:, None, :, :, :], align_corners=True).permute(
                0, 2, 3, 4, 1
            )
            ...
            if "multaccum" in renderoptions and renderoptions["multaccum"]:
                contrib = torch.exp(-lograyalpha) * (1.0 - torch.exp(-sample_alpha * dt))

                rayrgb = rayrgb + sample_rgb * contrib
                lograyalpha = lograyalpha + sample_alpha * dt
            else:
                contrib = (rayalpha + sample_alpha * dt).clamp(max=1.0) - rayalpha
        ...
        if "multaccum" in renderoptions and renderoptions["multaccum"]:
            rayalpha = 1.0 - torch.exp(-lograyalpha)
