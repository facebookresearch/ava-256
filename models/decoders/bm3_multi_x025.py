...
class DecoderSlab(nn.Module):
    def __init__(self, imsize, nboxes, boxsize, outch, viewcond=False, texwarp=False, enable_id_encoder=True, cfg=None):
        ...
        if imsize == 1024:
            size = [inch, 64, 32, 32, 16, 16, 16, 16, self.boxsize * self.outch]
            scale_factor = 4
        elif imsize == 512:
...
class DecoderGeoSlab2(nn.Module):
    def __init__(self, uv, tri, uvtri, nvtx, motion_size, geo_size, imsize, nboxes, boxsize, enable_id_encoder=True, cfg=None):
        ...
        if imsize == 1024:
            size = [inch, 64, 32, 32, 16, 16, 16, 16, self.boxsize]
            scale_factor = 4
        elif imsize == 512:
        ...
        for i in range(self.nlayers):
            ...
            if h == motion_size:
                self.motion = s(v(size[i+1], 16, 1, 1, 0), a(0.2, inplace=True), v(16, 9, 1, 1, 0))

            if h == geo_size:
                self.geo = s(v(size[i+1], 16, 1, 1, 0), a(0.2, inplace=True), v(16, 3, 1, 1, 0))
