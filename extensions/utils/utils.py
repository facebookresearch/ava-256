...
class ComputeRaydirs(Function):
    ...
    def forward(self, viewpos, viewrot, focal, princpt, pixelcoords, volradius):
        ...
        utilslib.compute_raydirs_forward(
            viewpos, viewrot, focal, princpt, pixelcoords, W, H, volradius, raypos, raydirs, tminmax
        )
...
def gradcheck():
    ...
    _viewpos = (
        torch.tensor([[-0.0, 0.0, -4.0] for n in range(N)], device="cuda") + torch.randn(N, 3, device="cuda") * 0.1
    )
    ...
    t1 = (-1.0 - viewpos[:, None, None, :]) / raydir
    t2 = (1.0 - viewpos[:, None, None, :]) / raydir
    tmin = torch.max(
        torch.min(t1[..., 0], t2[..., 0]),
    ...
    print(
        "{:<10} {:>10.5} {:>10.5} {:>10} {:>10.5} {:>10.5}".format(
            "fwd",
